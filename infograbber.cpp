...
InfoGrabber::InfoGrabber()
    ...
    connect(&mTracklistPage, SIGNAL(loadFinished(bool)), this, SLOT(tracklistSiteLoaded(bool)));
}

void InfoGrabber::grabStreamInfos() {
    openSite();
}

void InfoGrabber::grabTracklist(int streamId) {
    openTracklistSite(streamId);
}
...
void InfoGrabber::openTracklistSite(int streamId) {
    mTracklistPage.mainFrame()->load(QString{"http://www.%1tracklist/"}.arg(Streams::getUrlFromStreamId(streamId).c_str()));
}
...
void InfoGrabber::parseTracklist() {
    ...
    while(std::getline(mTracklistData, line)) {
        if(line.empty() || line == "Tracks" || (mlk::stl_string::contains("Charts", line) && line.length() == 10))
            continue;
        if(line == "Bookmarks")
            break;
